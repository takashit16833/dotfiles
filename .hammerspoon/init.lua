...
-- Mouse4 / Mouse5 のイベントを、マウスカーソル下ではなく
-- 現在フォーカスされているウィンドウへ届ける。
...
-- 再投入したイベントをこの eventtap 自身が再び横取りしないための印。
...
mouseNavigationEventTap = hs.eventtap.new({
  ...
  -- 物理カーソルは動かさず、イベント上の座標だけをアクティブウィンドウ中央へ移す。
  redirectedEvent:location({
    x = frame.x + frame.w / 2,
    y = frame.y + frame.h / 2,
  })
  redirectedEvent:setProperty(properties.eventSourceUserData, redirectedMouseEventMarker)

  -- アプリ固有のショートカットへ変換せず Mouse4 / Mouse5 のまま macOS へ再投入する。
  -- これにより各アプリが本来持っている戻る・進む等の解釈をそのまま使える。
  redirectedEvent:post()
